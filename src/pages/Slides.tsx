...
import {
  ArrowRight, FileText, MessageSquareText, Palette, Repeat,
  ChevronLeft, ChevronRight, Clock, Users, TrendingUp, Search,
  Target, Database, LayoutDashboard, FolderOpen, Package, Briefcase,
  ClipboardList, Play, Pause, Maximize, Minimize,
} from 'lucide-react'
import Logo from '../components/Logo'

/* ── Animated decor backgrounds ── */
const decorVariants: Record<number, (t: 'light' | 'dark') => React.ReactNode> = {
  0: (theme) => (
...
function StatCard({ icon: Icon, value, label, sub, accent }: {
  icon: any; value: string; label: string; sub: string; accent?: boolean
}) {
  return (
...
/* ── Slide definitions (memoized) ── */
function useSlides() {
  return useMemo(() => {
    const processColumns = [
      { no: '01', title: '需求池归类', icon: Search, visual: 'needs' as const, caption: '从实际工作中归纳出关键问题',
        items: [['资料分散', '评论、商品、客服反馈散落在不同文件'], ['岗位断裂', '各岗位分析口径与输出格式不统一'], ['经验不可交接', 'AI 用法停留在个人经验中'], ['重复整理', '每次大促都从零启动']] },
      { no: '02', title: '定义与策略', icon: Target, visual: 'strategy' as const, caption: '将问题转化为产品定义与核心策略',
        items: [['资料结构化', '统一资料类型与字段'], ['岗位任务化', '明确目标、输入与输出格式'], ['AI 可控化', '结构化分析 + 质量检查'], ['流程资产化', '沉淀为 Work Kit']] },
      { no: '03', title: '系统架构设计', icon: Database, visual: 'architecture' as const, caption: '将策略转化为系统结构与功能模块',
        items: [['电商资料库', '集中管理评论、商品、客服、文案'], ['岗位分析任务', '生成任务卡与 Prompt'], ['AI 分析工作台', '承载结果与质量检查'], ['报告与资产库', '汇总并保存流程资产']] },
      { no: '04', title: '界面设计', icon: LayoutDashboard, visual: 'interface' as const, caption: '将系统结构转化为用户可操作的界面',
        items: [['Dashboard', '项目总览与进度看板'], ['资料库', '导入与管理电商资料'], ['任务卡', '展示岗位任务和输出要求'], ['AI 工作台', '生成结构化分析结果'], ['报告页', '输出可执行策略报告']] },
    ]
    ...
    return [
      /* 1 - Cover */
      {
        ...
        content: (
          <div className="flex flex-col items-center text-center max-w-2xl">
            ...
            <h1 className="text-[clamp(40px,8vw,68px)] font-light tracking-[-0.03em] text-white mb-3 animate-fade-in-up">PromoKit AI</h1>
            <p className="text-[clamp(14px,2vw,18px)] text-white/45 mb-12 animate-fade-in-up" style={{ animationDelay: '100ms' }}>电商大促 AI 工作包系统</p>
            <p className="text-[clamp(13px,1.8vw,15px)] text-white/28 max-w-md leading-relaxed animate-fade-in-up" style={{ animationDelay: '250ms' }}>把一次有效的大促分析流程<br />沉淀为团队可复用的 AI 工作包</p>
      ...
      /* 2 - Problem */
      {
        ...
        content: (
          <div className="max-w-[680px] w-full">
            ...
            <h2 className="text-[clamp(28px,5vw,44px)] font-light tracking-[-0.02em] text-text-main mb-5 leading-[1.15] animate-fade-in-up">大促前的准备<br />团队总是从头来过</h2>
            <p className="text-[15px] text-text-muted mb-10 leading-relaxed animate-fade-in-up" style={{ animationDelay: '80ms' }}>五个岗位各自收集信息、各自分析、各自输出——<span className="text-accent-600 font-medium">口径不一，结果难以对齐</span>。</p>
            <div className="grid grid-cols-2 gap-3 mb-8 stagger">
              {[
                ...
                <div key={item.role} className="animate-fade-in-up bg-bg-surface rounded-2xl p-5 border border-border-default flex items-start gap-4 transition-shadow duration-300 hover:shadow-md">
                  <div className="w-10 h-10 rounded-xl bg-accent-50 flex items-center justify-center shrink-0"><item.icon className="w-5 h-5 text-accent-500" /></div>
                  <div>
            ...
            <div className="bg-sidebar rounded-2xl p-5 text-center animate-fade-in-up" style={{ animationDelay: '350ms' }}>
              <p className="text-[15px] text-white/70 leading-relaxed">结果：下次大促，<span className="text-accent-400 font-semibold">所有人从零开始再来一遍</span>。</p>
            </div>
          </div>
        ),
      },
      /* 3 - Cost */
      {
        ...
        content: (
          <div className="max-w-[680px] w-full">
            ...
            <h2 className="text-[clamp(28px,5vw,44px)] font-light tracking-[-0.02em] text-text-main mb-5 leading-[1.15] animate-fade-in-up">团队经验<br />正在悄悄流失</h2>
            <p className="text-[15px] text-text-muted mb-10 leading-relaxed animate-fade-in-up" style={{ animationDelay: '80ms' }}>每次大促的分析流程、判断标准和输出模板，都只存在个人脑子里。<span className="text-accent-600 font-medium">人一走，经验就没了</span>。</p>
            ...
            <p className="text-[14px] text-text-muted text-center animate-fade-in-up leading-relaxed" style={{ animationDelay: '350ms' }}>团队并不缺 AI 工具——<span className="text-accent-600 font-semibold">缺的是把 AI 分析流程化、资产化的系统</span>。</p>
          </div>
        ),
      },
      /* 4 - Solution */
      {
        ...
        content: (
          <div className="max-w-[720px] w-full">
            ...
            <h2 className="text-[clamp(28px,5vw,44px)] font-light tracking-[-0.02em] text-text-main mb-4 leading-[1.15] animate-fade-in-up">用 PromoKit AI<br />把流程变成资产</h2>
            <p className="text-[15px] text-text-muted mb-10 leading-relaxed animate-fade-in-up" style={{ animationDelay: '80ms' }}>同一份数据源 → 五个岗位协同分析 → 结构化输出 → <span className="text-accent-600 font-semibold">一键沉淀为可复用工作包</span></p>
      ...
      /* 5 - Workflow (dark) */
      {
        ...
        content: (
          <div className="max-w-[760px] w-full">
            ...
            <h2 className="text-[clamp(28px,5vw,44px)] font-light tracking-[-0.02em] text-white mb-12 leading-[1.15] animate-fade-in-up">六步完成闭环<br />最后一步才是关键</h2>
            <div className="flex items-start gap-0 stagger mb-12">
              {[
                ...
                <div key={item.step} className="flex-1 flex items-start animate-fade-in-up">
                  ...
                  {i < arr.length - 1 && <div className="pt-6 -ml-1 mr-1 opacity-40"><ArrowRight className="w-3 h-3 text-white" /></div>}
                </div>
              ))}
            </div>
            <div className="bg-white/4 rounded-2xl p-5 text-center animate-fade-in-up" style={{ animationDelay: '450ms' }}>
              <p className="text-[14px] text-white/55 leading-relaxed">第 6 步沉淀的 Work Kit，让下一次的第 1 步<br />从<span className="text-white/35 line-through">"从零开始"</span> 变成 <span className="text-accent-400 font-semibold">"几分钟启动"</span>。</p>
            </div>
          </div>
        ),
      },
      /* 6 - Differentiator */
      {
        ...
        content: (
          <div className="max-w-[680px] w-full">
            ...
            <h2 className="text-[clamp(28px,5vw,44px)] font-light tracking-[-0.02em] text-text-main mb-4 leading-[1.15] animate-fade-in-up">我们保存的不是结果<br />而是流程本身</h2>
            <p className="text-[15px] text-text-muted mb-10 leading-relaxed animate-fade-in-up" style={{ animationDelay: '80ms' }}>每个 Work Kit 记录的是<span className="text-accent-600 font-semibold">资料结构、任务模板、Prompt 和报告格式</span>。换一个品类、换一次大促，照样能用。</p>
            <div className="bg-bg-surface rounded-[24px] p-7 border border-border-default shadow-sm mb-6 animate-scale-in" style={{ animationDelay: '150ms' }}>
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-3">
                  ...
                  <div><div className="text-[15px] font-semibold text-text-main">618 竞品评论分析 Work Kit</div><div className="text-[11px] text-text-muted">v1.2 · 已复用 3 次</div></div>
                </div>
            ...
            <p className="text-[14px] text-text-muted text-center animate-fade-in-up leading-relaxed" style={{ animationDelay: '300ms' }}>每次大促后迭代一个版本——<span className="text-accent-600 font-semibold">团队经验永不丢失</span>。</p>
          </div>
        ),
      },
      /* 7 - Before/After */
      {
        ...
        content: (
          <div className="max-w-[680px] w-full">
            ...
            <h2 className="text-[clamp(28px,5vw,44px)] font-light tracking-[-0.02em] text-text-main mb-8 leading-[1.15] animate-fade-in-up">用数据说话</h2>
      ...
      /* 8 - Design Process */
      {
        ...
        label: '设计',
        content: (
          <ProcessMappingSlide processColumns={processColumns} />
        ),
      },
      /* 9 - CTA */
      {
        ...
        content: (
          <div className="flex flex-col items-center text-center max-w-xl">
            ...
            <h2 className="text-[clamp(36px,6vw,48px)] font-light tracking-[-0.02em] text-white mb-2 leading-[1.15] animate-fade-in-up" style={{ animationDelay: '120ms' }}>PromoKit AI</h2>
            ...
            <p className="text-[15px] text-white/35 max-w-sm leading-relaxed mb-10 animate-fade-in-up" style={{ animationDelay: '300ms' }}>把一次有效的大促分析流程<br />变成团队下一次可直接复用的起点</p>
            <div className="flex items-center gap-4 animate-fade-in-up" style={{ animationDelay: '400ms' }}>
              <Link to="/" className="btn-primary-filled bg-bg-surface text-accent-600 border-white hover:bg-bg-surface/90 hover:border-white text-[15px] px-8 py-3.5 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-white/10">进入产品演示 <ArrowRight className="w-4 h-4" /></Link>
              <Link to="/create" className="btn-primary text-white border-white/25 hover:bg-white/8 hover:border-white/50 text-[15px] px-8 py-3.5 transition-all duration-300">创建第一个项目 <ArrowRight className="w-4 h-4" /></Link>
            </div>
...
function ProcessVisual({ type }: { type: string }) {
  if (type === 'needs') {
    return (
      <div className="h-[94px] rounded-[18px] border border-accent-100 bg-white/65 px-4 py-3 flex items-center gap-4">
        <div className="w-16 h-16 rounded-2xl border border-accent-100 bg-bg-surface flex items-center justify-center"><Search className="w-9 h-9 text-accent-500" strokeWidth={2.2} /></div>
        <div className="flex-1 space-y-2">
          {[['竞品评论', 'bg-accent-500'], ['客服问答', 'bg-emerald-500'], ['历史文案', 'bg-amber-500']].map(([text, color]) => (
            <div key={text as string} className="h-7 rounded-lg border border-border-default bg-bg-surface px-3 flex items-center gap-2 text-[11px] font-medium text-text-secondary">
              <span className={`w-2 h-2 rounded-full ${color as string}`} />{text as string}
            </div>
          ))}
        </div>
      </div>
  ...
  if (type === 'strategy') {
    return (
      <div className="h-[94px] rounded-[18px] border border-accent-100 bg-white/65 px-4 py-3 flex items-center gap-4">
        <div className="w-16 h-16 rounded-full border border-accent-100 bg-bg-surface flex items-center justify-center"><Target className="w-10 h-10 text-accent-500" strokeWidth={2.3} /></div>
  ...
  if (type === 'architecture') {
    const modules: [any, string, string][] = [[Database, '资料库', 'text-blue-500'], [ClipboardList, '任务卡', 'text-emerald-600'], [Briefcase, '工作台', 'text-accent-500'], [Package, '资产库', 'text-red-500']]
    return (
      <div className="h-[94px] rounded-[18px] border border-accent-100 bg-white/65 px-4 py-3 grid grid-cols-2 gap-2">
        {modules.map(([Icon, text, color]) => (
          <div key={text} className="rounded-xl border border-border-default bg-bg-surface px-3 flex items-center gap-2">
            <Icon className={`w-4 h-4 ${color}`} strokeWidth={2.3} />
  ...
  return (
    <div className="h-[112px] rounded-[18px] border border-accent-100 bg-white/65 px-4 py-3 grid grid-cols-2 grid-rows-2 gap-2.5 overflow-hidden">
      {([[LayoutDashboard, 'Dashboard'], [FolderOpen, '资料库'], [Briefcase, '工作台'], [FileText, '报告页']] as const).map(([Icon, text]) => (
        <div key={text} className="min-h-0 rounded-xl border border-border-default bg-bg-surface px-3 py-1.5 shadow-[0_8px_18px_rgba(28,28,30,0.03)]">
          <div className="flex items-center gap-2 mb-1.5"><Icon className="w-4 h-4 text-accent-500" strokeWidth={2.3} /><span className="text-[11px] font-bold text-text-secondary">{text}</span></div>
...
function ConnectorLabel({ text, className = '' }: { text: string; className?: string }) {
  return (
    <div className={`absolute flex items-center gap-3 text-[12px] font-bold text-accent-600 ${className}`}>
      <span>{text}</span><span className="h-px w-12 bg-accent-400/70" /><ArrowRight className="w-4 h-4" strokeWidth={2.2} />
    </div>
  )
}

function ProcessCard({ column }: { column: typeof processColumnsData[number] }) {
  ...
  return (
    <div className="h-[452px] rounded-[18px] border border-accent-200/80 bg-bg-surface/95 shadow-[0_20px_45px_rgba(28,28,30,0.07)] overflow-hidden">
      <div className="h-[58px] border-b border-accent-200/70 px-5 flex items-center justify-between bg-gradient-to-b from-white to-accent-50/25">
        ...
        <div className="w-9 h-9 rounded-xl border border-accent-100 bg-bg-surface flex items-center justify-center"><Icon className="w-4.5 h-4.5 text-accent-500" strokeWidth={2.3} /></div>
      </div>
      <div className="p-4">
...
function ProcessMappingSlide({ processColumns }: { processColumns: typeof processColumnsData }) {
  return (
    <div className="relative w-[1180px] max-w-[calc(100vw-58px)] -my-12 pt-2 pb-2">
      ...
      <div className="relative animate-scale-in" style={{ animationDelay: '80ms' }}>
        ...
        <div className="grid grid-cols-4 gap-4">
          {processColumns.map((column) => <ProcessCard key={column.no} column={column} />)}
        </div>
      ...
      <div className="relative mt-11 mx-auto w-[850px] h-[74px] rounded-2xl border border-accent-200/80 bg-bg-surface/95 shadow-[0_18px_45px_rgba(28,28,30,0.06)] flex items-center px-7 animate-fade-in-up" style={{ animationDelay: '180ms' }}>
        <div className="w-12 h-12 rounded-2xl border border-accent-100 bg-accent-50/40 flex items-center justify-center shrink-0"><TrendingUp className="w-6 h-6 text-accent-500" strokeWidth={2.3} /></div>
        <div className="h-10 w-px bg-accent-200 mx-7" />
        <div className="text-[20px] font-bold text-accent-600 shrink-0 mr-7">最终目标</div>
        <p className="text-[14px] leading-relaxed text-text-main">把一次有效的大促分析流程，转化为团队可复用、可交接、可迭代的 AI 工作包资产，持续提升团队效率与决策质量。</p>
      </div>
    </div>
  )
}
...
const slideTransition = (dir: Direction): React.CSSProperties => {
  if (!dir) return { animation: 'fade-in-up 0.35s ease-out' }
  if (dir === 'next') return { animation: 'slide-in-right 0.35s ease-out' }
  return { animation: 'slide-in-left 0.35s ease-out' }
}
...
export default function Slides() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setCurrent((p) => {
        if (p >= total - 1) { setAutoplay(false); return p }
        setDirection('next')
        return p + 1
      })
    }, 6000)
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape' && autoplay) { setAutoplay(false) }
    }
    window.addEventListener('keydown', handler)
    return () => window.removeEventListener('keydown', handler)
  }, [goNext, goPrev, toggleFullscreen, autoplay])
  ...
  return (
    <div
      ...
      className={`slides-container min-h-screen flex items-center justify-center relative overflow-hidden transition-colors duration-700 ${slide.theme === 'dark' ? 'bg-sidebar' : 'bg-bg-primary'} ${isFullscreen ? 'fullscreen' : ''}`}
      onTouchStart={handleTouchStart}
      onTouchEnd={handleTouchEnd}
      onWheel={handleWheel}
    >
      {decorVariants[slide.bgVariant]?.(slide.theme)}

      <div key={current} className="relative z-10 px-8 py-16" style={slideTransition(direction)}>
        {slide.content}
      </div>
      ...
      {/* Nav arrows */}
      <button onClick={goPrev} disabled={current === 0} aria-label="上一页"
        className={`absolute left-6 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 disabled:opacity-0 ${slide.theme === 'dark' ? 'bg-white/5 hover:bg-white/12' : 'bg-black/3 hover:bg-black/6'}`}>
        <ChevronLeft className={`w-4 h-4 ${slide.theme === 'dark' ? 'text-white/45' : 'text-text-muted'}`} />
      </button>
      <button onClick={goNext} disabled={current === total - 1} aria-label="下一页"
        className={`absolute right-6 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 disabled:opacity-0 ${slide.theme === 'dark' ? 'bg-white/5 hover:bg-white/12' : 'bg-black/3 hover:bg-black/6'}`}>
      ...
      {/* Control bar: autoplay + fullscreen */}
      <div className={`absolute top-6 right-8 flex items-center gap-2 ${slide.theme === 'dark' ? 'text-white/30' : 'text-text-muted'}`}>
        <button
          onClick={toggleAutoplay}
          title={autoplay ? '暂停自动播放' : '自动播放'}
          aria-label={autoplay ? '暂停自动播放' : '自动播放'}
          className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200 ${autoplay ? (slide.theme === 'dark' ? 'bg-white/10 text-accent-400' : 'bg-accent-50 text-accent-600') : (slide.theme === 'dark' ? 'hover:bg-white/8' : 'hover:bg-black/5')}`}
        >
        ...
        <button
          ...
          className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200 ${slide.theme === 'dark' ? 'hover:bg-white/8' : 'hover:bg-black/5'}`}
        >
