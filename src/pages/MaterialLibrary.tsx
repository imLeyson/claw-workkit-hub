...
export default function MaterialLibrary() {
  ...
  const handleSaveComp = () => {
    ...
    const comp: Competitor = {
      ...
      brand: compForm.brand.trim() || compForm.name.trim(),
      platform: compForm.platform as any,
      price: compForm.price || '待定价',
      reviewCount: parseInt(compForm.reviewCount) || 0,
      rating: parseFloat(compForm.rating) || 0,
      topIssues: compForm.topIssues ? compForm.topIssues.split('|').map((s) => s.trim()).filter(Boolean) : [],
    }
    const updated = editingIndex !== null
      ? competitors.map((c, i) => (i === editingIndex ? comp : c))
      : [...competitors, comp]
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (type === 'review' && file.name.includes('竞品') && competitors.length === 0) {
      ...
      setCompetitors(detected); saveCompetitors(detected)
      showToast(`已导入并自动识别 ${detected.length} 个竞品`, 'success')
    } else {
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-border-default rounded-[24px] p-8 text-center mb-10 hover:border-accent-300 hover:bg-accent-50/20 transition-all cursor-pointer group">
        ...
        <p className="text-[12px] text-text-muted mb-4">支持 CSV · Excel · PDF — 上传竞品评论数据自动识别商品</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
        ...
        {competitors.length === 0 && (
          <div className="col-span-full text-center py-10 border-2 border-dashed border-border-default rounded-2xl">
            ...
            <button onClick={openAddComp} className="text-[12px] font-medium text-accent-600 hover:text-accent-700">
              + 手动添加竞品
            </button>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl p-6 w-[480px] max-w-[90vw] shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-medium text-text-muted mb-1.5">商品名称 *</label>
                  <input type="text" value={compForm.name} onChange={(e) => setCompForm({ ...compForm, name: e.target.value })} placeholder="例：米家高速吹风机 H700" className="w-full text-[13px] px-3 py-2.5 border border-border-default rounded-xl focus:outline-none focus:border-accent-400" autoFocus />
                </div>
                <div>
                  <label className="block text-[11px] font-medium text-text-muted mb-1.5">品牌</label>
                  <input type="text" value={compForm.brand} onChange={(e) => setCompForm({ ...compForm, brand: e.target.value })} placeholder="例：小米米家" className="w-full text-[13px] px-3 py-2.5 border border-border-default rounded-xl focus:outline-none focus:border-accent-400" />
                </div>
              </div>

              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-[11px] font-medium text-text-muted mb-1.5">平台</label>
                  <select value={compForm.platform} onChange={(e) => setCompForm({ ...compForm, platform: e.target.value })} className="w-full text-[13px] px-3 py-2.5 border border-border-default rounded-xl focus:outline-none focus:border-accent-400 bg-white">
                    {PLATFORMS.map((p) => <option key={p} value={p}>{p}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-[11px] font-medium text-text-muted mb-1.5">价格</label>
                  <input type="text" value={compForm.price} onChange={(e) => setCompForm({ ...compForm, price: e.target.value })} placeholder="¥499" className="w-full text-[13px] px-3 py-2.5 border border-border-default rounded-xl focus:outline-none focus:border-accent-400" />
                </div>
                <div>
                  <label className="block text-[11px] font-medium text-text-muted mb-1.5">评论数</label>
                  <input type="number" value={compForm.reviewCount} onChange={(e) => setCompForm({ ...compForm, reviewCount: e.target.value })} placeholder="426" className="w-full text-[13px] px-3 py-2.5 border border-border-default rounded-xl focus:outline-none focus:border-accent-400" />
                </div>
              </div>

              <div>
                <label className="block text-[11px] font-medium text-text-muted mb-1.5">好评率 (%)</label>
                <input type="number" value={compForm.rating} onChange={(e) => setCompForm({ ...compForm, rating: e.target.value })} placeholder="91.4" step="0.1" min="0" max="100" className="w-24 text-[13px] px-3 py-2.5 border border-border-default rounded-xl focus:outline-none focus:border-accent-400" />
              </div>

              <div>
                <label className="block text-[11px] font-medium text-text-muted mb-1.5">高频差评问题</label>
                <input type="text" value={compForm.topIssues} onChange={(e) => setCompForm({ ...compForm, topIssues: e.target.value })} placeholder="噪音偏大 | 风嘴松动 | 发热明显（用 | 分隔）" className="w-full text-[13px] px-3 py-2.5 border border-border-default rounded-xl focus:outline-none focus:border-accent-400" />
                <p className="text-[10px] text-text-muted mt-1">多个问题用 "|" 分隔，如：噪音偏大 | 发热明显</p>
              </div>
            </div>

            <div className="flex items-center justify-end gap-3 mt-6">
              <button onClick={() => setShowCompForm(false)} className="btn-ghost text-[13px]">取消</button>
              <button onClick={handleSaveComp} className="btn-primary-filled text-[13px]">
                {editingIndex !== null ? '保存修改' : '添加竞品'}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-2">
        ...
        {materials.length === 0 && <div className="text-center py-10 text-[13px] text-text-muted">暂无资料，上传文件开始</div>}
      </div>
    </div>
  )
}
