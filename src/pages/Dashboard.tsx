  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="mb-16">
        ...
        <h1 className="text-[40px] font-light leading-[1.15] tracking-[-0.02em] text-text-main mb-5 max-w-[580px]">
          竞品评论分析<br />AI 工作流系统
        </h1>
      ...
      <div className="grid grid-cols-3 gap-5 mb-20">
        {mockProjects.map((p) => (
          <Link key={p.id} to={p.status === 'in_progress' ? `/materials/${p.id}` : p.status === 'completed' ? `/report/${p.id}` : `/tasks/${p.id}`} className="card-surface rounded-[24px] card-hover overflow-hidden group flex flex-col">
